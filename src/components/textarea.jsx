...
function Text()
{
    ...
    var time = 0.008 * text.split(" ").length;
    const handleUpClick = ()=>{
    ...
    return  (
    <>
    <div className="textarea mx-6">
    <h1 className="text-2xl mt-5 text-center">Enter Text to Analyze</h1>
    <textarea
    placeholder="Enter Your Text Here"
    value={text}
    onChange={handleOnChange}
      className="resize-none w-full p-3 text-md border-solid border-2 border-black rounded-lg mt-3"
      rows="8"
      id="message"
    ></textarea>
    
    
    <span class="md:inline-grid grid-cols-3 gap-4 px-64">
    <button onClick={handleUpClick} className="flex mt-4 text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-700 rounded text-lg">Convert To UpperCase</button>
    <button onClick={handleLoClick} className="flex mt-4 text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-700 rounded text-lg">Convert To LowerCase</button>
    <button onClick={clear} className="flex mt-4 text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-700 rounded text-lg">Clear The Entered Text</button>
</span>
    </div>

    <div className="container  edit">

    <h1 className="text-3xl text-center my-3 mt-5">Your Text Summary</h1>
    ...
    <div className="px-6 pt-6 2xl:container">
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div className="md:col-span-2 lg:col-span-1" >
                <div className="h-96 py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white">
                    ...
                    <div>
                        <h5 className="text-xl text-gray-600 text-center">Number of Words</h5>
                        <div className="mt-2 flex justify-center gap-4">
                            <textarea className=" resize-none text-3xl font-bold text-gray-700 text-center" value={text.split(" ").length}></textarea>
                           </div>
                           <h5 className="text-xl text-gray-700 text-center">Number of Characters</h5>
                    <div className="mt-2 flex justify-center gap-4">
                            <textarea className=" resize-none text-3xl font-bold text-gray-700 text-center" value={text.length}></textarea>
                           </div>
                           </div>
                           </div>
            </div>
            <div>
                <div className="h-full py-6 px-6 rounded-xl border border-gray-200 bg-white">
                    <h5 className="text-xl text-gray-700 text-center">Time to Read</h5>
                    <div className="mt-2 flex justify-center gap-4">
                            <textarea className=" resize-none text-3xl font-bold text-gray-700 text-center" value={time.toFixed(2)}></textarea>
                           <p className="text-md"> Minutes</p>
                           </div>
                    <table className="mt-6 -mb-2 w-full text-gray-600">
                        <tbody>
                            <tr>
                                <td className="py-2">Average Speed</td>
                                <td className="text-gray-500">220 WPM</td>
                                <td>
                            ...
                            <tr>
                                <td className="py-2">Fast Speed</td>
                                <td className="text-gray-500">350 WPM</td>
                                <td>
                                    <svg className="w-16 ml-auto" viewBox="0 0 68 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <rect opacity="0.4" width="17" height="21" rx="1" fill="#e4e4f2"/>
                                        <rect opacity="0.4" x="19" width="14" height="21" rx="1" fill="#e4e4f2"/>
